            ...
            className='flex-1'>
              <p className='mb-10 max-w-2xl font-Ovo'
                      ...
                      I have had the privilege of collaborating with students, contributing to their success and growth.</p>
                          ...
                          sm:grid-cols-3 gap-6 max-w-2x1'>
                              {infoList.map(({icon, iconDark, title,
                              description}, index)=>(
                              <motion.li 
                              whileInView={{scale:1.05}}
                              className='border-[0.5px]
                              border-gray-400 rounded-xl p-6
                              cursor-pointer hover:bg-lightHover hover:-translate-y-1 duration-500 hover:shadow-black dark:border:white 
                              dark:hover:shadow-white dark:hover:bg-darkHover/50'
                              key={index}>
                                <Image src={isDarkMode ? icon: iconDark} alt={title}
                                className='w-7 mt-3'/>
                                <h3 className='my-4
                                font-semibold text-gray-700 dark:text-white'>
                                {title}</h3>
                                <p className='text-gray-600
                                text-sm dark:text-white/80'>{description}</p>
                              </motion.li>
                              ))}
                              </motion.ul>
                ...
                className='my-6 text-gray-700 font-Ovo dark:text-white/80'>Tools I use</motion.h4>
                               ...
                               className='flex items-center gap-3 sm:gap-5'>
                                ...
                                rounded-lg cursor-pointer hover:-translate-y-1 I
                                duration-500'
                                key={index}>
